             ...
             if ($status) {
                 ...
                 $height = 0;
                     ...
                     foreach ($data_types as $type){
                         ...
                         if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/shipping/qwqer.tpl')) {
                             $template = $this->load->view($this->config->get('config_template') . '/template/extension/shipping/qwqer.tpl', array('text_title_order_type' => $type, 'langs'=>$lang));
                         } else {
                             $template = $this->load->view('default/template/extension/shipping/qwqer.tpl', array('text_title_order_type' => $type, 'langs'=>$lang));
                         }

                         //$template = $this->load->view('extension/shipping/qwqer.tpl', array('text_title_order_type' => $type, 'langs'=>$lang));
                     ...
                     if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/shipping/qwqer_title.tpl')) {
                         $template = $this->load->view($this->config->get('config_template') . '/template/extension/shipping/qwqer_title.tpl', array('current_price'=>$price, 'text_title' => $this->language->get('text_title'), 'token'=>$token, 'langs'=>$lang,'url'=>$url,'moduleType'=>$moduleType));
                     } else {
                         $template = $this->load->view('default/template/extension/shipping/qwqer_title.tpl', array('current_price'=>$price, 'text_title' => $this->language->get('text_title'), 'token'=>$token, 'langs'=>$lang,'url'=>$url,'moduleType'=>$moduleType));
                     }
                     $method_data = array(
                         'code' => 'qwqer.standart',
                         'title' => $template,
